...
// Duplicado a propósito de Planeación/Producción (mismo criterio de esas
// páginas Cancelados) — pero acá SOLO cuenta ítems con
// estado_liberacion = 'enviado_a_produccion', igual que /calidad/pedidos/[id]:
// a Calidad no le interesan ítems que nunca llegaron a su flujo.
export default async function CanceladosCalidadPage() {
  ...
  // Solo listar pedidos que de hecho tienen algo cancelado que le importe a
  // Calidad: el PM completo, o al menos un ítem enviado a producción.
        ...
        <tbody className="divide-y divide-slate-100">
          {pedido.items.map((it) => (
            <tr key={it.id}>
              ...
              <td className="px-3 py-2">{it.folio ?? "sin evaluar"}</td>
              {puedeRevertir && (
                <td className="px-3 py-2">
