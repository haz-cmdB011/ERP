-- Amplía ingest_planeacion_version (versión actual: 20260915154747, ya con
-- categoria_componente e imágenes) para persistir además las banderas de
-- validación y las fases de taller nuevas (ver
-- 20260915170000_planeacion_liberacion_produccion.sql). estado_liberacion /
-- liberado_en / liberado_por NO se tocan aquí: nacen en 'pendiente' y solo
-- cambian vía la acción de liberación a producción (API route).
...
begin
  ...
  loop
    ...
    -- imagen_paths: rutas ya subidas a Storage por route.ts antes de esta
    -- llamada (el buffer binario de la imagen no es jsonb-safe). El orden
    -- del arreglo se conserva vía with ordinality.
